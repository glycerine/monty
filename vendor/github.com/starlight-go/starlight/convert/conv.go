	...
	return starlark.NewBuiltin(name, func(thread *starlark.Thread, fn *starlark.Builtin, args starlark.Tuple, kwargs []starlark.Tuple) (starlark.Value, error) {
		...
		for i, v := range vals {
			...
			if !val.Type().AssignableTo(argT) {
				val, ok, err = tryConvert(val, argT)
				if !ok {
					fmt.Printf("converstion failed, argT = '%#v', val='%#v', err='%v'\n", argT, val, err)
					return nil, err
				}
			}
			rvs = append(rvs, val)
...
func tryConvert(val reflect.Value, argT reflect.Type) (ret reflect.Value, ok bool, err error) {
	defer func() {
		r := recover()
		if r != nil {
			// conversion failed
			// check for starlark.NoneType -> nil pointer
			_, isNoneType := val.Interface().(starlark.NoneType)
			if isNoneType && argT.Kind() == reflect.Ptr {
				//fmt.Printf("None being assigned to pointer\n")
				ret = reflect.New(argT.Elem()) // nil pointer
				ok = true
				err = nil
			} else {
				err = fmt.Errorf("error in Conversion, recovered: '%#v'", r)
			}
		}
	}()

	tmpRet := val.Convert(argT)
	// ok!
	ret = tmpRet
	ok = true
	return
